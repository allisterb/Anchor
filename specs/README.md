...
| `DogwoodSemantics.tla` | our reading of Dogwood's temporal operators — `formerly`, `previous`, `since`, the `count`/`sum` aggregations, and the partitioning an event schema's `pin` imposes. Differential-tested against Dogwood's own corpus on 776 pairs. |
