\* GENERATED by src/translator/policy_module.py from approval_gate_error.dw -- do not edit.
\*
...
Policies ==
  <<
    [effect |-> "permit", actions |-> {"Trade"}, cond |-> [op |-> "term", args |-> <<>>, term |-> [op |-> "formerly", window |-> 1000, atom |-> [op |-> "pred", pred |-> [action |-> "Approve", kind |-> "error", binds |-> <<>>], var |-> "", args |-> <<>>, field |-> "", cmp |-> "", value |-> [k |-> "s", v |-> ""], other |-> "", pattern |-> <<>>, net |-> <<>>, prefix |-> 0], left |-> [op |-> "pred", pred |-> [action |-> "Approve", kind |-> "error", binds |-> <<>>], var |-> "", args |-> <<>>, field |-> "", cmp |-> "", value |-> [k |-> "s", v |-> ""], other |-> "", pattern |-> <<>>, net |-> <<>>, prefix |-> 0], leftNeg |-> FALSE, keys |-> <<>>]]]
  >>
