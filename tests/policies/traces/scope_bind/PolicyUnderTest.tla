\* GENERATED by src/translator/policy_module.py from scope_bind.dw -- do not edit.
\*
...
Policies ==
  <<
    [effect |-> "permit", actions |-> {"Trade"}, cond |-> [op |-> "term", args |-> <<>>, term |-> [op |-> "formerly", window |-> 3600, atom |-> [op |-> "pred", pred |-> [action |-> "Approve", kind |-> "request", binds |-> <<[side |-> "scope", field |-> "callerPrincipal", kind |-> "scope", name |-> "principal", value |-> [k |-> "s", v |-> ""]]>>], var |-> "", args |-> <<>>, field |-> "", cmp |-> "", value |-> [k |-> "s", v |-> ""], other |-> "", pattern |-> <<>>, net |-> <<>>, prefix |-> 0], left |-> [op |-> "pred", pred |-> [action |-> "Approve", kind |-> "request", binds |-> <<[side |-> "scope", field |-> "callerPrincipal", kind |-> "scope", name |-> "principal", value |-> [k |-> "s", v |-> ""]]>>], var |-> "", args |-> <<>>, field |-> "", cmp |-> "", value |-> [k |-> "s", v |-> ""], other |-> "", pattern |-> <<>>, net |-> <<>>, prefix |-> 0], leftNeg |-> FALSE, keys |-> <<>>]]]
  >>
