---------------------------- MODULE TrustDecay ----------------------------
\* What `07-trust-decay.dw` is SUPPOSED to mean, taken word for word from the article that
...
\* The article's own sentence. After 15 minutes with no advisor interaction, a write is refused.
LosesWriteAfter15m ==
...
\* The claim asked about separately -- a TIGHTER deadline than the policy states. It is stated as
\* its own invariant rather than folded into the one above, because "does this policy also enforce
\* ten minutes" is a different question from "does it do what it says", and an answer that
\* conflated them would be useless for either.
LosesWriteAfter10m ==
